...
def image_processing(pfp: BytesIO, name: str, content: str):
    with Image.new(mode="RGBA", size=(800, 300), color=(0, 0, 0)) as base:
        ...
        with Pilmoji(base) as final:
            final.text((250, 100 if len(content) < 100 else 20), fill(content, width=60, max_lines=12), font=font_content, align="left")
...
class MakeQuote(lightbulb.MessageCommand, name="Create Quote"):
    ...
    async def invoke(
        ...
        pfp = self.target.author.make_avatar_url(file_format="WEBP", size=4096, lossless=True) or self.target.author.default_avatar_url
        user_pfp = BytesIO()
        ...
        img = await loop.run_in_executor(pool, image_processing, user_pfp, self.target.author.display_name, self.target.content.strip())
